...
export default async function OrderDetailPage({
  ...
  searchParams?: Promise<{ deleteError?: string; whatsapp?: string; whatsappSent?: string }>;
}) {
  await requireAdmin();
    ...
    <main className="admin-page">
      <section className="admin-shell">
        ...
        <div className="admin-panels detail-panels">
          ...
          <div className="admin-card whatsapp-zone"><h2>WhatsApp</h2><p>Open a prefilled customer message when payment is confirmed or the order is received.</p><div className="admin-detail-actions"><AdminWhatsAppLink order={order} intent="payment_confirmed" label="Payment message" returnTo={`/admin/orders/${order.order_code}?whatsappSent=payment_confirmed`} /><AdminWhatsAppLink order={order} intent="delivered" label="Received message" returnTo={`/admin/orders/${order.order_code}?whatsappSent=delivered`} /></div></div>
